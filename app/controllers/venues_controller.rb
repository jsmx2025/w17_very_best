class VenuesController < ApplicationController
  def index
    ...
    @venues = @q.result(:distinct => true).includes(:foodvenues, :foods).page(params[:page]).per(10)
    @location_hash = Gmaps4rails.build_markers(@venues.where.not(:location_latitude => nil)) do |venue, marker|
      ...
      marker.infowindow "<h5><a href='/venues/#{venue.id}'>#{venue.place}</a></h5><small>#{venue.location_formatted_address}</small>"
    end
